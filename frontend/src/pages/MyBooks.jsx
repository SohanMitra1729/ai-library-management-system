        ...
        <div className="flex-1 relative">
          {loading ? (
            <div className="absolute inset-0 flex flex-col justify-center items-center">
              ...
              <p className="text-textSecondary font-bold tracking-widest uppercase text-sm">Fetching your library history...</p>
            </div>
          ) : error ? (
            ...
            >
              <div className="w-24 h-24 bg-bgPrimary/50 border border-white/10 rounded-full flex items-center justify-center mb-6 shadow-inner">
                <Book className="text-textSecondary" size={48} />
              </div>
              <h3 className="text-2xl font-black text-textPrimary mb-2">No borrowed books found</h3>
              <p className="text-textSecondary text-lg max-w-md font-medium">You haven't borrowed any books yet. Check out the catalog to find your next great read!</p>
            </motion.div>
          ) : (
            <motion.div 
                ...
                {books.map((book, index) => (
                  ...
                  >
                    <div className="h-64 bg-bgSecondary flex items-center justify-center relative overflow-hidden transition-colors border-b border-white/5">
                      ...
                      <div className="w-40 h-56 rounded-r-xl border-l-[6px] border-bgPrimary shadow-[15px_15px_30px_rgba(0,0,0,0.8)] flex flex-col justify-between transform group-hover:rotate-2 group-hover:scale-110 transition-all duration-500 relative overflow-hidden bg-bgPrimary">
                        ...
                        <div className="absolute bottom-2 left-2 right-2">
                          <h4 className="text-textPrimary text-xs font-black line-clamp-2 leading-tight drop-shadow-md">{book.title}</h4>
                        </div>
                      </div>
                    ...
                    <div className="p-6 flex flex-col flex-1 bg-bgPrimary/30">
                      ...
                      <p className="text-sm font-bold text-textSecondary uppercase tracking-wider mb-4">{book.author}</p>
                      
                      <div className="space-y-3 mb-4 mt-auto">
                        <div className="flex justify-between items-center text-sm border-b border-white/5 pb-2">
                          <span className="text-textSecondary flex items-center gap-1 font-bold"><Clock size={14}/> Issue Date</span>
                          <span className="text-textPrimary font-mono">{new Date(book.issue_date).toLocaleDateString()}</span>
                        </div>
                        <div className="flex justify-between items-center text-sm border-b border-white/5 pb-2">
                          <span className="text-textSecondary flex items-center gap-1 font-bold"><Clock size={14}/> Due Date</span>
                          <span className="text-textPrimary font-mono">{new Date(book.due_date).toLocaleDateString()}</span>
                        </div>
                        {book.return_date && (
                          <div className="flex justify-between items-center text-sm border-b border-white/5 pb-2">
                            <span className="text-textSecondary flex items-center gap-1 font-bold"><CheckCircle2 size={14}/> Return Date</span>
                            <span className="text-accentCyan font-mono">{new Date(book.return_date).toLocaleDateString()}</span>
                          </div>
                        )}
                        <div className="flex justify-between items-center text-sm pt-2">
                          <span className="text-textSecondary font-bold">Fine Amount</span>
                          <span className={`font-mono font-bold text-lg ${book.fine_amount > 0 ? 'text-red-400' : 'text-textPrimary'}`}>
